...
export default function DownloadPaymentsModal() {
  ...
  const handleDownload = async () => {
    try {
      ...
      const path = `/api/v1/payment/download?${queryString}`;
      ...
      link.download = `payments_export_${new Date().getTime()}.${extension}`;
      link.click();

      URL.revokeObjectURL(url);
      toast.success(`Payments downloaded as ${format.toUpperCase()}`);
  ...
  return (
    <Modal
      ...
      dialogTitle="Download Payments Data"
